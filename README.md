...
``` javascript
var createQuery = require( 'aws-ec2-metrics' );
```

#### createQuery( opts[, clbk] )

Creates a new `Query` instance for retrieving metrics of `running` EC2 instances from [AWS]().
...
-	__tags__: `object array` specifying EC2 instance tags by which to filter running instances. Each `object` should consist of `key` and `value` properties, where `key` is the tag and `value` is the tag value.
	...
	// Return metrics for only those running instances having the `app` tag with a value `amazing_app`...
	var query = createQuery( opts );
	query.on( 'data', onData );
...
-	__interval__: positive `number` defining a poll [interval](https://developer.mozilla.org/en-US/docs/Web/API/WindowTimers/setInterval) for repeatedly querying AWS. The interval should be in units of `milliseconds`. If an `interval` is __not__ provided, only a single query is made. Default: `300000` (5min).
	...
	// Every hour, fetch the metrics of running EC2 instances...
	var query = createQuery( opts );
	query.on( 'data', onData );
...
*	Multiple `tags` are supported. Key-value pairs should be separated by an `=` symbol. Keys are assumed to be free of any `=` symbols.
	...
	``` bash
	$ aws-ec2-metrics --key <key> --secret <secret> --tag 'beep=boop' --tag 'woot=be bop'
	```
...
``` bash
$ aws-ec2-metrics --key <key> --secret <secret>
# => '[<id>,<id>,...]'
```
...
``` bash
$ AWS_ACCESS_KEY=<key> AWS_ACCESS_SECRET=<secret> aws-ec2-metrics
# => '[<id>,<id>,...]'
```
...
``` bash
$ ./node_modules/.bin/aws-ec2-metrics --key <key> --secret <secret>
```
...
[npm-image]: http://img.shields.io/npm/v/aws-ec2-metrics.svg
[npm-url]: https://npmjs.org/package/aws-ec2-metrics

[travis-image]: http://img.shields.io/travis/kgryte/aws-ec2-metrics/master.svg
[travis-url]: https://travis-ci.org/kgryte/aws-ec2-metrics

[codecov-image]: https://img.shields.io/codecov/c/github/kgryte/aws-ec2-metrics/master.svg
[codecov-url]: https://codecov.io/github/kgryte/aws-ec2-metrics?branch=master

[dependencies-image]: http://img.shields.io/david/kgryte/aws-ec2-metrics.svg
[dependencies-url]: https://david-dm.org/kgryte/aws-ec2-metrics

[dev-dependencies-image]: http://img.shields.io/david/dev/kgryte/aws-ec2-metrics.svg
[dev-dependencies-url]: https://david-dm.org/dev/kgryte/aws-ec2-metrics

[github-issues-image]: http://img.shields.io/github/issues/kgryte/aws-ec2-metrics.svg
[github-issues-url]: https://github.com/kgryte/aws-ec2-metrics/issues
